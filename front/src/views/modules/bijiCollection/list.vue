        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '笔记名称' : ''">
                        <el-input prefix-icon="el-icon-search" v-model="searchForm.bijiName" placeholder="笔记名称" clearable></el-input>
                    </el-form-item>
                                                                                                     
                    <el-form-item :label="contents.inputTitle == 1 ? '笔记类型' : ''">
                        <el-select v-model="searchForm.bijiTypes" placeholder="请选择笔记类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                                    v-for="(item,index) in bijiTypesSelectSearch"
                                    v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('bijiCollection','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/biji/upload/bijiCollectionMuBan.xls"
                        >批量导入笔记收藏数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('bijiCollection','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="bijiCollectionUploadSuccess"
                                :on-error="bijiCollectionUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('bijiCollection','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入笔记收藏数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('bijiCollection','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "bijiCollection.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="bijiPhoto"
                               header-align="center"
                               width="200"
                               label="笔记照片">
                        <template slot-scope="scope">
                            <div v-if="scope.row.bijiPhoto">
                                <img :src="$base.url+scope.row.bijiPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="bijiCollectionTypes"
                                      header-align="center"
                                      label="类型">
                        <template slot-scope="scope">
                            {{scope.row.bijiCollectionValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('bijiCollection','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('bijiCollection','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('bijiCollection','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'biji_collection',//当前表表名,
                        sumColum : 'biji_collection_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'biji_collection_name',//分组字符串字段
                        // types : 'biji_collection_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "biji_collection_types",
                //// riqi :getMonthFormat(_this.echartsDate),//年月
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                let params = {
                    ...
                    limit: this.pageSize,
                    sort: 'id',
                    bijiCollectionTypes: 1,
                }

                                                             
                if (this.searchForm.bijiName!= '' && this.searchForm.bijiName!= undefined) {
                    params['bijiName'] = '%' + this.searchForm.bijiName + '%'
                }
                                                                                                     
                if (this.searchForm.bijiTypes!= '' && this.searchForm.bijiTypes!= undefined) {
                    params['bijiTypes'] = this.searchForm.bijiTypes
                }
                ...
                params['bijiCollectionDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "bijiCollection/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
                ...
                //查询级联表搜索条件所有列表
                this.$http({
                    url: "dictionary/page?dicCode=biji_types&page=1&limit=100",
                    method: "get",
                    page: 1,
                    limit: 100,
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.bijiTypesSelectSearch = data.data.list;
                    }
                });
                this.$http({
            ...
            // 导入功能上传文件成功后调用导入方法
            bijiCollectionUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "bijiCollection/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            bijiCollectionUploadError(data){
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "bijiCollection/update",
                        method: "post",
                        data: {
                            id:id,
//                            bijiCollectionTypes:bijiCollectionTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
