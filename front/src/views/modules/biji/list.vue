                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('biji','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/biji/upload/bijiMuBan.xls"
                        >批量导入笔记广场数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('biji','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="bijiUploadSuccess"
                                :on-error="bijiUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('biji','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入笔记广场数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('biji','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "biji.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
                ...
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('biji','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('biji','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('biji','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
    ...
    //当前表下拉框搜索条件
        ...
        methods: {
            ...
            getDataList() {
                ...
                params['bijiDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            bijiUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "biji/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
