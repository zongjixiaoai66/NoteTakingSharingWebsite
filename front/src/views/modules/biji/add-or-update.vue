    ...
    <div class="addEdit-block">
        <el-form
            ...
            <input id="yonghuId" name="yonghuId" type="hidden">
               <el-col :span="12">
                   <el-form-item class="input" v-if="type!='info'"  label="笔记名称" prop="bijiName">
                       <el-input v-model="ruleForm.bijiName"
                                 placeholder="笔记名称" clearable  :readonly="ro.bijiName"></el-input>
                   </el-form-item>
                   <div v-else-if="type=='info'">
               ...
               <el-col :span="12">
                   <el-form-item class="input" v-if="type!='info'"  label="笔记编号" prop="bijiUuidNumber">
                       <el-input v-model="ruleForm.bijiUuidNumber"
                                 placeholder="笔记编号" clearable  :readonly="ro.bijiUuidNumber"></el-input>
                   </el-form-item>
                   <div v-else-if="type=='info'">
                       <el-form-item class="input" label="笔记编号" prop="bijiUuidNumber">
                           <el-input v-model="ruleForm.bijiUuidNumber"
                                     placeholder="笔记编号" readonly></el-input>
                       </el-form-item>
                   </div>
               </el-col>
                <el-col :span="12">
                    <el-form-item class="upload" v-if="type!='info' && !ro.bijiPhoto" label="笔记照片" prop="bijiPhoto">
                        <file-upload
                            ...
                            :limit="3"
                            :multiple="true"
                            :fileUrls="ruleForm.bijiPhoto?$base.url+ruleForm.bijiPhoto:''"
                            @change="bijiPhotoUploadChange"
                        ></file-upload>
                    </el-form-item>
                    <div v-else>
               ...
               <el-col :span="12">
                ...
                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="笔记类型" prop="bijiTypes">
                        <el-select v-model="ruleForm.bijiTypes" :disabled="ro.bijiTypes" placeholder="请选择笔记类型">
                            <el-option
                                v-for="(item,index) in bijiTypesOptions"
                ...
                <el-col :span="24">
                    <el-form-item v-if="type!='info' && !ro.bijiContent"  label="笔记内容" prop="bijiContent">
                        <editor style="min-width: 200px; max-width: 600px;"
                                v-model="ruleForm.bijiContent"
                                class="editor"
                                action="file/upload">
                        </editor>
                    </el-form-item>
                    <div v-else-if="type=='info'">
                        <el-form-item v-if="ruleForm.bijiContent" label="笔记内容" prop="bijiContent">
                            <span v-html="ruleForm.bijiContent"></span>
                        </el-form-item>
                    </div>
                </el-col>
            </el-row>
...
<script>
    ...
    export default {
        data() {
            return {
                ...
                ruleForm: {
                    yonghuId: '',
                    bijiName: '',
                    bijiUuidNumber: new Date().getTime(),
                    bijiPhoto: '',
                    zanNumber: 0,
                    caiNumber: 0,
                    bijiTypes: '',
                ...
                rules: {
                   ...
                   bijiUuidNumber: [
                              { required: true, message: '笔记编号不能为空', trigger: 'blur' },
                          ],
                   bijiPhoto: [
                              { required: true, message: '笔记照片不能为空', trigger: 'blur' },
                          ],
                   zanNumber: [
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=biji_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.bijiTypesOptions = data.data.list;
                    }
                });
        ...
        methods: {
            ...
            info(id) {
                ...
                _this.$http({
                    ...
                    if (data && data.code === 0) {
                        ...
                        _this.ruleForm.bijiContent = _this.ruleForm.bijiContent.replaceAll("src=\"upload/","src=\""+this.$base.url+"upload/");
                    } else {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.ruleForm.bijiContent = this.ruleForm.bijiContent.replaceAll(this.$base.url,"");
                        this.$http({
                            url:`biji/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.bijiCrossAddOrUpdateFlag = false;
                                        this.parent.search();
            ...
            //图片
            bijiPhotoUploadChange(fileUrls){
                this.ruleForm.bijiPhoto = fileUrls;
                this.addEditUploadStyleChange()
            },
