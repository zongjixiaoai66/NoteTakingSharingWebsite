        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('guanzhu','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/biji/upload/guanzhuMuBan.xls"
                        >批量导入我的关注数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('guanzhu','导入导出')"
                                ...
                                :on-error="guanzhuUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('guanzhu','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入我的关注数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('guanzhu','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "guanzhu.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="guanzhuUuidNumber"
                                   header-align="center"
                                   label="关注编号">
                        <template slot-scope="scope">
                            {{scope.row.guanzhuUuidNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="guanzhuKucunNumber"
                                      header-align="center"
                                      label="关注人">
                        <template slot-scope="scope">
                            {{scope.row.guanzhuKucunNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('guanzhu','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('guanzhu','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('guanzhu','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            <!--<el-button v-if="isAuth('guanzhu','修改')" type="primary" icon="el-icon-add-location" size="mini" @click="plusGuanzhuKucunNumber(scope.row.id,scope.row.guanzhuKucunNumber)">增加关注人</el-button>-->
                            <!--<el-button v-if="isAuth('guanzhu','修改')" type="primary" icon="el-icon-delete-location" size="mini" @click="reduceGuanzhuKucunNumber(scope.row.id,scope.row.guanzhuKucunNumber)">减少关注人</el-button>-->
...
<!-- 库存 -->
		<el-dialog
                :title="guanzhuKucunNumberTitle"
                :visible.sync="guanzhuKucunNumberVisible"
                width="30%">
            <span>操作数量</span><el-input-number v-model="kucunNumber" :min="1" :max="100" label="描述文字"></el-input-number>
            <span slot="footer" class="dialog-footer">
			<el-button @click="guanzhuKucunNumberVisible = false">取 消</el-button>
			<el-button type="primary" @click="addGuanzhuKucunNumber()">确 定</el-button>
		  </span>
        </el-dialog>
...
<script>
    ...
    //当前表下拉框搜索条件
            form:{
                id : null,
                yonghuId : null,
                guanzhuUuidNumber : null,
                guanzhuKucunNumber : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '账户状态': 'jinyongTypes',
                //本表字段
                     '关注编号': "guanzhuUuidNumber",
                     '关注人': "guanzhuKucunNumber",
                     '关注时间': "insertTime",
            },
        ...
        //库存
            ...
            //原有数量
            guanzhuKucunNumber: 0,
            //标题
            guanzhuKucunNumberTitle: null,
            //模态框状态
            guanzhuKucunNumberVisible: false,
            };
        },
        created() {
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'guanzhu',//当前表表名,
                        sumColum : 'guanzhu_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'guanzhu_name',//分组字符串字段
                        // types : 'guanzhu_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['guanzhuDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            guanzhuUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "guanzhu/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            guanzhuUploadError(data){
                this.$message.error('上传失败');
            },
            //增加关注人
			plusGuanzhuKucunNumber(thisId,guanzhuKucunNumber){
			//清空数据
				//关注人数据设置为0
				this.kucunNumber = 0
				//id清空
			...
			//数据不为空放值
				...
				//将模态框打开
				this.guanzhuKucunNumberVisible = true;
				//标题
				this.guanzhuKucunNumberTitle = '增加关注人'
			},
			//减少关注人
			reduceGuanzhuKucunNumber(thisId,guanzhuKucunNumber){
			//清空数据
				//关注人数据设置为0
				this.kucunNumber = 0
				//id清空
			...
			//数据不为空放值
				...
				this.guanzhuKucunNumberVisible = true;
				//标题
				this.guanzhuKucunNumberTitle = '减少关注人'
			},
			//提交用户操作关注人数量
			addGuanzhuKucunNumber(){
				var type = null
				var maxNumber = 0
				if(this.kucunNumberTypes == "+"){
					type = '增加'
					maxNumber = this.guanzhuKucunNumber + this.kucunNumber
				}else{
					type = '减少'
					if(this.guanzhuKucunNumber < this.kucunNumber || this.kucunNumber <= 0){
						this.$message.error('关注人不足不能减少');
						return false;
					}else{
						maxNumber = this.guanzhuKucunNumber - this.kucunNumber
					}
				...
				this.guanzhuKucunNumberVisible = false;
			},	            //无用
            wuyong(id) {
                let _this = this;
